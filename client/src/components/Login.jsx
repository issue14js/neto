...
const Login = () => {
    ...
    return (
        <form className={`h-100  p-5 rounded-2xl w-70 bg-zinc-800"  ${themeStyles} `} onSubmit={handelSubmit}>
            ...
            <input required className="border-white mb-2 border rounded p-2 w-full text-white outline-none" type="text" placeholder="Enter Username" name="username" onChange={handelChange} />
            <span className="block text-white">Password</span>
            <input required className="border-white mb-2 block w-full border rounded p-2 text-white outline-none" type="password" placeholder="Enter Password" name="password" onChange={handelChange} />
            <button type="submit" className={`w-full mt-2  border py-2 px-4 rounded text-white bg-zinc-500 ${themeStyles}`} >Login</button>
